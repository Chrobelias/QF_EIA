(set-logic ALL)
(declare-fun i1 () Int)
(declare-fun it1238 () Int)
(declare-fun it1239 () Int)
(declare-fun i2 () Int)
(declare-fun i3 () Int)
(declare-fun it1240 () Int)
(declare-fun i4 () Int)
(declare-fun it1241 () Int)
(declare-fun it1242 () Int)
(declare-fun i5 () Int)
(declare-fun i6 () Int)
(declare-fun it1243 () Int)
(declare-fun i7 () Int)
(declare-fun it1244 () Int)
(declare-fun it1245 () Int)
(declare-fun it1246 () Int)
(declare-fun it1247 () Int)
(declare-fun it1248 () Int)
(declare-fun it1249 () Int)
(declare-fun it1250 () Int)
(declare-fun it1251 () Int)
(declare-fun it1398 () Int)
(declare-fun it1399 () Int)
(declare-fun it1400 () Int)
(declare-fun it1401 () Int)
(declare-fun it1402 () Int)
(declare-fun it1403 () Int)
(declare-fun it1404 () Int)
(declare-fun it1405 () Int)
(declare-fun it1406 () Int)
(declare-fun it1407 () Int)
(declare-fun it1408 () Int)
(declare-fun it1409 () Int)
(declare-fun it1410 () Int)
(declare-fun it1411 () Int)
(declare-fun it1415 () Int)
(declare-fun it1416 () Int)
(declare-fun it1417 () Int)
(declare-fun it1418 () Int)
(declare-fun it1419 () Int)
(declare-fun it1420 () Int)
(declare-fun it1421 () Int)
(declare-fun it1422 () Int)
(declare-fun it1423 () Int)
(declare-fun it1424 () Int)
(declare-fun it1425 () Int)
(declare-fun it1426 () Int)
(declare-fun it1427 () Int)
(declare-fun it1428 () Int)
(declare-fun it1474 () Int)
(declare-fun it1475 () Int)
(declare-fun it1476 () Int)
(declare-fun it1477 () Int)
(declare-fun it1478 () Int)
(declare-fun it1479 () Int)
(declare-fun it1480 () Int)
(declare-fun it1482 () Int)
(declare-fun it1483 () Int)
(declare-fun it1484 () Int)
(declare-fun it1485 () Int)
(declare-fun it1486 () Int)
(declare-fun it1487 () Int)
(declare-fun it1488 () Int)
(declare-fun it1490 () Int)
(declare-fun it1491 () Int)
(declare-fun it1492 () Int)
(declare-fun it1493 () Int)
(declare-fun it1494 () Int)
(declare-fun it1495 () Int)
(declare-fun it1496 () Int)
(declare-fun it101 () Int)
(assert (= (+ i1 (- 1)) 0))
(assert (= (+ it1238 (- 5)) 0))
(assert (= (+ (* i2 (- 1)) it1239 (- 4)) 0))
(assert (= (+ it1240 (* i3 (- 1))) 0))
(assert (= (+ it1241 (* i4 (- 1))) 0))
(assert (= (+ (* i5 (- 1)) it1242) 0))
(assert (= (+ it1243 (* i6 (- 1))) 0))
(assert (= (+ it1244 (* i7 (- 1))) 0))
(assert (and (= (+ it1238 (- 5)) 0) (> (+ it1241 (- 2)) 0)))
(assert (= (+ it1245 (- 18)) 0))
(assert (= (+ (* it1239 (- 1)) it1246 (- 12)) 0))
(assert (= (+ (* it1240 (- 1)) it1247) 0))
(assert (= (+ (* it1241 (- 1)) it1248) 0))
(assert (= (+ it1249 (* it1242 (- 1))) 0))
(assert (= it1250 0))
(assert (= (+ (* it1244 (- 1)) it1251) 0))
(assert (let ((a!1 (>= (+ (+ (* it1250 (- 1)) it1248) (- 4)) 0))
      (a!2 (>= (+ (+ (* it1250 (- 1)) it1248) (- 3)) 0))
      (a!3 (>= (+ (+ (* it1250 (- 1)) it1248) (- 2)) 0))
      (a!4 (> (+ (+ (* it1250 (- 1)) it1248) (- 3)) 0)))
  (and (> 1 0)
       a!1
       (>= (+ it1245 (- 18)) 0)
       (>= (+ (* it1245 (- 1)) 18) 0)
       (> (+ it1250 it1248 3) 0)
       a!2
       a!3
       a!4)))
(assert (= (+ (* it1245 (- 1)) it1398) 0))
(assert (= (+ it1399 (* it1250 11) (* it1248 (- 11)) (* it1246 (- 1)) 33) 0))
(assert (= (+ it1400 (* it1248 (- 1)) 3) 0))
(assert (= (+ (* it1248 (- 1)) it1401) 0))
(assert (= (+ it1402 (* it1248 (- 1))) 0))
(assert (= (+ it1403 (* it1248 (- 1)) 3) 0))
(assert (= (+ it1404 (- 1)) 0))
(assert (and (= (+ it1398 (- 18)) 0) (<= (+ it1403 (* it1401 (- 1)) 2) 0)))
(assert (= (+ it1405 (- 20)) 0))
(assert (= (+ (* it1399 (- 1)) it1406 (- 2)) 0))
(assert (= (+ (* it1400 (- 1)) it1407) 0))
(assert (= (+ (* it1401 (- 1)) it1408) 0))
(assert (= it1409 0))
(assert (= (+ (* it1403 (- 1)) it1410) 0))
(assert (= (+ (* it1404 (- 1)) it1411) 0))
(assert (and (<= (+ (* it1409 2) it1410 (* it1408 (- 1)) 3) 0) (= (+ it1405 (- 20)) 0)))
(assert (= (+ it1415 (- 21)) 0))
(assert (= (+ (* it1406 (- 1)) it1416 (- 1)) 0))
(assert (= (+ (* it1407 (- 1)) it1417) 0))
(assert (= (+ it1418 (* it1408 (- 1))) 0))
(assert (= (+ (* it1409 (- 1)) it1419) 0))
(assert (= (+ (* it1410 (- 1)) it1420) 0))
(assert (= (+ it1421 (* it1411 (- 1))) 0))
(assert (and (= (+ (* it1418 (- 1)) (* it1419 2) it1420 3) 0) (= (+ it1415 (- 21)) 0)))
(assert (= (+ it1422 (- 26)) 0))
(assert (= (+ (* it1416 (- 1)) it1423 (- 2)) 0))
(assert (= (+ (* it1417 (- 1)) it1424) 0))
(assert (= (+ (* it1418 (- 1)) it1425) 0))
(assert (= (+ (* it1419 (- 1)) it1426) 0))
(assert (= (+ (* it1420 (- 1)) it1427) 0))
(assert (= (+ it1428 (* it1419 (- 2)) (- 1)) 0))
(assert (= (+ it1422 (- 26)) 0))
(assert (= (+ it1474 (- 20)) 0))
(assert (= (+ it1475 (* it1423 (- 1)) (- 2)) 0))
(assert (= (+ it1476 (* it1424 (- 1))) 0))
(assert (= (+ (* it1425 (- 1)) it1477) 0))
(assert (= (+ it1478 (* it1428 (- 1))) 0))
(assert (= (+ it1479 (* it1427 (- 1))) 0))
(assert (= (+ (* it1428 (- 1)) it1480) 0))
(assert (and (> (+ (* it1478 2) it1479 (* it1477 (- 1)) 3) 0) (= (+ it1474 (- 20)) 0)))
(assert (= (+ it1482 (- 18)) 0))
(assert (= (+ (* it1475 (- 1)) it1483 (- 4)) 0))
(assert (= (+ (* it1479 (- 1)) it1484 (- 1)) 0))
(assert (= (+ it1485 (* it1477 (- 1))) 0))
(assert (= (+ (* it1478 (- 1)) it1486) 0))
(assert (= (+ (* it1479 (- 1)) it1487 (- 1)) 0))
(assert (= (+ it1488 (* it1480 (- 1))) 0))
(assert (and (= (+ it1482 (- 18)) 0) (<= (+ (* it1485 (- 1)) it1487 2) 0)))
(assert (= (+ it1490 (- 20)) 0))
(assert (= (+ it1491 (* it1483 (- 1)) (- 2)) 0))
(assert (= (+ it1492 (* it1484 (- 1))) 0))
(assert (= (+ (* it1485 (- 1)) it1493) 0))
(assert (= it1494 0))
(assert (= (+ it1495 (* it1487 (- 1))) 0))
(assert (= (+ (* it1488 (- 1)) it1496) 0))
(assert (let ((a!1 (* (* (+ 0 it1494) (exp 2 (+ it101 (- 1)))) (+ 0 (- 2))))
      (a!3 (* (* (+ 0 it1494) (exp 2 (+ it101 (- 1)))) (+ 0 2))))
(let ((a!2 (+ (* (exp 2 (+ it101 (- 1))) (+ 0 (- 2)))
              a!1
              (+ 0 (* it1495 (- 1)))
              (+ 0 it1493)
              (+ 0 (- 1))))
      (a!4 (+ (* (exp 2 (+ it101 (- 1))) (+ 0 2))
              a!3
              (+ 0 it1495)
              (+ 0 (* it1493 (- 1)))
              (+ 0 1))))
  (and (>= (+ (* it1494 (- 2)) (* it1495 (- 1)) it1493 (- 3)) 0)
       (>= (+ (* it1490 (- 1)) 20) 0)
       (>= a!2 (+ 0 0))
       (>= (+ it101 (- 1)) 0)
       (>= (+ (* it1494 2) it1495 (* it1493 (- 1)) 3) 0)
       (>= (+ it1490 (- 20)) 0)
       (>= a!4 (+ 0 0))))))
(check-sat)
